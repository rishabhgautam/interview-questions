...
| CROSS | Cartesian product | Use carefully — explosion risk |
...
- Inclusive: `x BETWEEN 10 AND 20` → `10 <= x <= 20`
...
🧠 Performance Tip:  
Large `IN (...)` lists → slow → move to a **temp lookup table** and join.

---

## 4. GROUP BY — Subtleties
...
## 5. Window Functions (Key Interview Topic)
...
🧠 Do not mix window functions directly with `WHERE` filters on the same alias → use subquery/CTE.

---

## 6. EXISTS vs IN — Which to Use?
...
| ----------- | ------------------- | ----------------------- |
| `UNION`     | Yes                 | ❌ (needs sorting/dedup) |
| `UNION ALL` | No                  | ✔ (preferred)           |
...
## 8. Primary Key vs Unique Key vs Foreign Key

| Type        | Allows NULL? | Purpose                        |
| ----------- | ------------ | ------------------------------ |
| Primary Key | ❌            | Row identity                   |
| Unique Key  | ✔ (usually)  | Uniqueness constraint          |
| Foreign Key | ✔            | Maintain referential integrity |

---

## 9. Indexing — Basics Interviewers Expect
...
* Don’t index fields with high update frequency → write amplification
* Composite index order matters → leftmost prefix rule

---

## 10. SQL Generation in RAG Systems

Pipeline:
1. Retrieve **schema + example queries**
2. Constrain generation to allowed **tables/columns**
3. Validate using:

* Query parser / dry-run (`EXPLAIN`)
* Automated **repair loop** on errors
  4. Log + store corrections → feedback improves future RAG performance
...
# Quick Interview Quiz (Answers Hidden Below)

**Q1:** Why can’t we use column aliases in WHERE?
...
* A2: `WHERE right.col IS NULL` → Left Anti Join
...
* Understand logical execution flow → fewer surprises
