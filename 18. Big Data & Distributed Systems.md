...
## 🧱 1️⃣ Big Data Concepts

---

### 🔹 A) Small-File vs Big-File Problem
...
**Best Practices**
- Prefer **128–1024 MB** file sizes
- Use **repartition / coalesce** to compact outputs  
- Store in **Parquet** instead of JSON/CSV
- Periodic compaction of streaming outputs (Kafka → Spark)

🧠 *Rule of Thumb:* If you have **millions of files**, performance is **dying**.

---

### 🔹 B) Why Parquet Rocks for Analytics

- **Columnar format** → read only needed fields → low I/O  
...
### 🔹 C) Spark Performance Pitfalls & Fixes
...
## 🌐 2️⃣ Distributed Systems Essentials

---

### 🔹 A) Redis — Where It Fits
...
🧠 If response must be **<1 ms**, Redis is your friend.

---

### 🔹 B) Kafka Pub-Sub — Mental Model
...
Core Properties:
- High throughput (MBs–GBs/sec)
- Message retention (not just queueing)
- Consumer-group scaling  
  → each partition consumed by **only 1** consumer in a group
...
🧠 Offsets = **reliability + replay**

---

### 🔹 C) How Consistent Hashing Works (Simple)

1️⃣ Hash nodes onto a ring  
2️⃣ Hash keys onto the ring  
3️⃣ Key owned by **first node clockwise**

When a node joins/leaves →  
only small region around it remaps → minimal movement
...
🧠 Works great for **caches** + **sharded NoSQL**.
...
|---|---|---|
| Round Robin | Simple | Doesn’t consider load |
...
Industry Tip:
> “Random with two choices” is surprisingly powerful and scalable.

---

### 🔹 E) NoSQL Indexing (High-level)
...
- **B-Trees** — ordered queries
- **LSM Trees** — write-optimized (Cassandra, Bigtable)

Trade-offs:
- **Local index** = fast write, slower cross-shard reads  
- **Global index** = fast reads, expensive write amplification

🧠 Always design **indexes based on queries**, not vice-versa.

---

### 🔹 F) Consistent Hashing vs Normal Hashing
...
## 🧪 3️⃣ Interview Questions — Practice
...
- Balance **# of partitions = 2–4 × # of cores**
- Distributed locks are always **tricky** → avoid if possible
...
> Big Data ≠ Big Results without smart **data layout + scheduling + indexing**.  
