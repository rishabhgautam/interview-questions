...
⚠ Only works if input meets constraints.

---

## 3️⃣ Maximum Product of 3 Numbers

Negative numbers matter!
Two negatives × one big positive can be best.
...
More optimal: track top3 & bottom2 in one scan → O(n).

---

## 4️⃣ Combinations: n Choose k (Backtracking Pattern)

💡 Use DFS to build combinations incrementally.
...
⏱ Time = O(C(n,k) * k) (to output everything)

---

## 5️⃣ Swap Without Temp Variable
...
👉 In real systems, tuple swap is safest + most readable.

---

## 6️⃣ Euclidean Distance Between Two Vectors
...
## 7️⃣ Bloom Filters (Must-Know Probabilistic Structure)
...
* k hash functions → set k bits per element
...
| Increase           | Effect                                     |
| ------------------ | ------------------------------------------ |
| Bit array size (m) | ↓ False positive rate                      |
| Hash functions (k) | Lower FPR until optimal point, then slower |

🚫 Cannot delete reliably (unless using Counting Bloom Filter)

---

## 8️⃣ XOR Filters — Bloom Alternative
...
* Fewer hash lookups → faster querying
...
| Static Data     | Yes ✔        | Best ✔         |
| Dynamic Updates | Limited ❌    | Poor ❌         |
| False Negatives | No           | No             |
| False Positives | Yes          | Yes (very low) |

Practical rule:

> Cloud + search + streaming → Bloom
> High-performance static index → XOR

---

# 🧠 Extra Interview Patterns (Rapid Fire)
...
1️⃣ Intersection: What if arrays are **huge** and streaming?
2️⃣ Duplicates: How to detect with **O(1) extra space**?
3️⃣ Max product: What if input length < 3?
4️⃣ Bloom filter: Formula for false positive probability?
5️⃣ XOR filter: Why unsuitable for dynamic datasets?
...
🎯 **Final takeaway:**
Mastering these patterns → 80% of coding interviews become predictable and solvable under pressure.
