...
- **L1 (Lasso):** Adds `λ * |w|` → pushes some weights to **zero** → automatic feature selection.  
- **L2 (Ridge):** Adds `λ * w²` → shrinks weights smoothly → helps prevent overfitting.  
- **When to use:** L1 for **sparse, high-dimensional data**, L2 for **stable, generalizable models**.  
- **Easy memory tip:** L1 = **Lasso = Less features**, L2 = **Ridge = Rounded weights**.
...
| High Variance | Too complex, sensitive to noise | Overfitting |

- Ideal goal → **Low Bias + Low Variance**.  
- Fix bias → increase complexity. Fix variance → simplify or regularize.  
- Memory tip: **Bias = Error from simplicity, Variance = Error from sensitivity**.
...
- **Gradient Descent:** Iteratively update weights in the **negative gradient direction** to minimize loss.
- **Non-convex landscapes (NNs):** Contain local minima, saddle points, and flat regions — hard to find a true global minimum.
- **Local vs Global:** Global is the lowest possible loss; local is lower than nearby points but not the absolute best.
- **Reality:** Modern deep nets often land in **good local minima** that generalize well — not necessary to reach the true global minima.
- **SGD helps escape traps:** Noise in stochastic updates helps avoid saddle points and escape sharp local minima.

**Mind-map handle:**  
Gradient path → Loss landscape → Valleys & pits → SGD noise helps → Good enough minima.
...
- **What it does:** Normalizes layer activations per mini-batch to mean 0 and variance 1, then scales (γ) and shifts (β).
- **Why useful:** Stabilizes training, speeds convergence, allows higher learning rates, reduces vanishing gradients.
- **Acts like regularizer:** Mini-batch noise adds slight randomness → reduces overfitting.
- **Mind hook:** “Normalize → then let the network learn its preferred scale and shift.”
