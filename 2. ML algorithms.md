...
**Pros:** Focuses on hard-to-classify samples, can turn weak learners (shallow trees) into a strong ensemble, good accuracy.  
**Cons:** Sensitive to noisy data and outliers, can overfit if too many iterations and weak learners are too complex.  
Builds learners sequentially, each correcting errors of the previous ones through reweighted samples.
...
**Autoencoder (AE):** Learns a deterministic mapping to a latent vector and back; minimizes reconstruction loss.  
**VAE:** Learns a distribution over latent variables (mean + variance) and adds a KL divergence term to regularize latent space.  
AEs are good for compression/denoising; VAEs are generative models that can sample new data from the latent distribution.  
VAEs encourage smooth, continuous latent spaces.
