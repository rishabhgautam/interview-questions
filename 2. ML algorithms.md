...
**Why linear?** Decision boundary is defined by `w^T x + b = 0`, which is a hyperplane, so it is a linear classifier.  
...
**Pros:** Easy to interpret and visualize, handles numeric and categorical features, captures non-linear boundaries, needs little preprocessing.  
...
**Splits:** Numeric features are split at thresholds (for example, `x < t`); categorical features are split by grouping categories into subsets.
